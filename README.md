...
После этого, факт того что операция применена, отправляется другим нодам и они применяют её на БД. 
...
* append он же heartbeat(если без данных) для репликации данных лога фоловерам и для предотвращения старта новог ораунда голосования. 
...
инициирует выборы, повышает номер раунда голосования и рассылает vote запросы другим нодамю 
...
Подключаем узлы, смотрим кто победил. **POST** http://localhost:8080/api/v1/context/start?peerId=2
...
Добавляем данные как описано в  пункте [штатаная репликация](#normal)
...
Через клиента данные добавить в отключенную ноду нельзя. 
Подключаемся напрямую к серверной ноде которую мы отключили.   
Если лидером был 2 (id = последняя цифроа порта) то http://localhost:8082/api/v1/swagger-ui.html 
...
Теперь добавляем данные в кластер уже через клиента как описано в пункте [штатаная репликация](#normal)

Таким образом мы получили два лидера оба с данными, у одного данные реплецированы и подтверждены большинством, 
у второго нет. 

Поднимаем отключенного лидера проверяем его лог и хранилище, там должны быть проавильные данные из кластера 
